import {
  ...
  function OngoingEventCard(props) {
      ...
      const submitHandler =async ()=>{
          if(scoreTeamA=== undefined || scoreTeamB===undefined || scoreTeamA===null || scoreTeamB===null) return;
          const idx = props.gameName.split(' ').findIndex(word => word.toLowerCase() === 'vs');
          const eventId = props.gameName.split(' ').slice(0, idx-1).join(' ');
          const subEventId = props.gameName.split(' ').slice(idx-1, idx+2).join(' ');
          const points ={
              teamA:{
                  name:props.teamA,
                  points:scoreTeamA,
          ...
          const body ={
              ...
              console.log(body);
          try{
              const resp = await axios.post(back_link+"api/event/updateLiveEvent" ,body);
              console.log(resp);
